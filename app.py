...
def _init():
    defaults = {'authenticated':False,'chat_history':[],'step':1,'temp_file_path':None,
                'scrapper':None,'uploaded_df':None,'case_num_col':None,'citation_col':None,
                'reconciled_data':None,'ai_assistant':None,'workers':8}
    for k,v in defaults.items():
        if k not in st.session_state: st.session_state[k]=v
    if st.session_state.ai_assistant is None:
        try: st.session_state.ai_assistant=AIAssistant()
        except: pass

def parallel_extract(scrapper,file_data,workers=8):
    import urllib.parse,json as _json
    ajax_hdrs={"X-Requested-With":"XMLHttpRequest","Content-Type":"application/x-www-form-urlencoded","Referer":"https://ilaw.kra.go.ke/ilaw/search/universal"}
    payload={"dataType":"litigation_data"}
    def fetch_one(item):
        try:
            resp=scrapper.session.post(f"{scrapper.url}{urllib.parse.quote(item['keyword'])}",data=payload,headers=ajax_hdrs,timeout=30)
            if resp.status_code!=200: return scrapper._empty_result(item)
            try: jd=resp.json()
            except _json.JSONDecodeError: return scrapper._empty_result(item)
            html=jd.get('html','')
            if not html: return {**scrapper._empty_result(item),'matches':[],'matches_found':0}
            matches=scrapper._parse_results(html)
            return {"excel_row":item.get('excel_row'),"original_case":item['case_number'],"case_name":item['citation'],"search_keyword":item['keyword'],"matches_found":len(matches),"matches":matches}
        except: return scrapper._empty_result(item)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in concurrent.futures.as_completed(futures):
            ...
            except: results[idx]=scrapper._empty_result(file_data[idx])
    return results

def login_page():
    st.markdown("<style>[data-testid='stSidebar']{display:none}.block-container{padding:0!important}</style>",unsafe_allow_html=True)
    _,col,_=st.columns([1,1.1,1])
    with col:
        st.markdown("<div style='height:7vh'></div>",unsafe_allow_html=True)
        logo_big=KRA_LOGO.replace('width="44"','width="62"').replace('height="44"','height="62"')
        st.markdown(f"""<div style='text-align:center;margin-bottom:2.2rem;'>{logo_big}
        ...
        st.markdown("<p style='color:#8a9099;font-size:.78rem;text-transform:uppercase;letter-spacing:.1em;margin-bottom:1.2rem;'>Sign in with your KRA credentials</p>",unsafe_allow_html=True)
        st.markdown("<p style='color:#8a9099;font-size:.73rem;text-transform:uppercase;letter-spacing:.1em;margin-bottom:2px;'>Username</p>",unsafe_allow_html=True)
        st.text_input("Username",placeholder="",key="lu",label_visibility="hidden")
        st.markdown("<p style='color:#8a9099;font-size:.73rem;text-transform:uppercase;letter-spacing:.1em;margin-bottom:2px;margin-top:.5rem;'>Password</p>",unsafe_allow_html=True)
        ...
        if login_btn:
            ...
            if not username or not password: st.error("Enter both username and password.")
            else:
                with st.spinner("Authenticating\u2026"):
                    sc=Scrapper(username=username,password=password)
                    if sc.authenticator():
                        st.session_state.scrapper=sc; st.session_state.authenticated=True; st.rerun()
                    else: st.error("Authentication failed. Check your credentials.")
        st.markdown("</div>",unsafe_allow_html=True)
        st.markdown("<div style='text-align:center;margin-top:1.4rem;color:#3e4550;font-size:.72rem;'>Use your authorised KRA iLaw credentials only.</div>",unsafe_allow_html=True)

        # ── Diagnostics (collapsed by default) ───────────────────────────
        with st.expander("🔬 Login not working? Run diagnostics"):
            st.markdown("<p style='color:#8a9099;font-size:.8rem;'>Enter your credentials below to see exactly what KRA iLaw returns — this reveals the real cause of any login failure.</p>",unsafe_allow_html=True)
            with st.form("diag_f"):
                ...
                if not _u or not _p:
                    ...
                    with st.spinner("Probing KRA iLaw…"):
                        try:
                            ...
                            st.table(rows)
                            # Build payload
                            hidden={}; ufield=None; pfield=None
                            SKIP={"redirect_to","csrf_token","_token","authenticity_token","token","submit"}
                            for i in soup.find_all("input"):
                                t=(i.get("type") or "text").lower(); n=(i.get("name") or "")
                                if t=="hidden" and n: hidden[n]=i.get("value","")
                                if t=="password" and not pfield: pfield=n
                                if t in("text","email") and n.lower() not in SKIP and not ufield: ufield=n
                            ufield=ufield or "username"; pfield=pfield or "password"
                            payload={ufield:_u, pfield:_p}; payload.update(hidden)
                            st.markdown(f"**POST payload keys:** `{list(payload.keys())}`")
                            r2=sess.post(AUTH_URL,data=payload,
                            ...
                            for tag in soup2(["script","style","head"]): tag.decompose()
                            body=" ".join(soup2.get_text(" ",strip=True).split())[:1000]
                            st.markdown("**Page content after POST:**")
                            st.code(body,language="text")
                            alerts=[e.get_text(strip=True) for e in soup2.find_all(class_=lambda c:c and any(x in c for x in ["alert","error","flash","message","notice","danger","warning"])) if e.get_text(strip=True)]
                            if alerts:
...
def sidebar_nav():
    with st.sidebar:
        st.markdown(f"""<div style='display:flex;align-items:center;gap:11px;padding:1rem .5rem .5rem;'>{KRA_LOGO}
            ...
            <div style='height:1px;background:var(--border);margin:1rem 0;'></div>""",unsafe_allow_html=True)
        ...
        if st.session_state.authenticated:
            ...
            st.markdown(f"""<div class='card-sm'><div style='font-size:.68rem;color:#8a9099;text-transform:uppercase;letter-spacing:.07em;'>Session</div>
                <div style='color:#7ec89b;font-size:.83rem;margin-top:3px;'>\u25cf Connected</div>
                <div style='font-size:.7rem;color:#8a9099;margin-top:2px;font-family:"Fira Code",monospace;'>{uname}</div></div>""",unsafe_allow_html=True)
...
def dashboard():
    ...
    st.markdown(f"""<div class='hero-banner'><div style='font-size:.72rem;color:#8a9099;text-transform:uppercase;letter-spacing:.12em;'>{greet}</div>
        <h1 style='margin:.15rem 0 .05rem;'>Reconciliation Dashboard</h1>
        <div class='hero-time'>{now}</div></div>""",unsafe_allow_html=True)
    ...
    def stat(col,lbl,val,sub,gold=False):
        col.markdown(f"""<div class='stat-card'><div class='stat-label'>{lbl}</div>
            ...
            <div class='stat-sub'>{sub}</div></div>""",unsafe_allow_html=True)
    if total:
    ...
    with ch1:
        ...
        if total:
            fig=go.Figure(go.Bar(x=['Verified','Review Req.','Mismatch','Not Found'],y=[verified,review,mismatch,nf],
                marker_color=['#5a8f6e','#b8903a','#c25f5f','#4a7fa5'],
                text=[verified,review,mismatch,nf],textposition='outside',textfont=dict(color='#8a9099',size=11)))
            fig.update_layout(plot_bgcolor='rgba(0,0,0,0)',paper_bgcolor='rgba(0,0,0,0)',
                font=dict(color='#8a9099',family='DM Sans'),height=250,margin=dict(l=0,r=0,t=10,b=0),showlegend=False,
                xaxis=dict(showgrid=False,color='#8a9099'),yaxis=dict(showgrid=True,gridcolor='#2e333a',color='#8a9099'))
    ...
    with ch2:
        ...
        if total:
            fig2=go.Figure(go.Pie(
                ...
                hole=.62,marker=dict(colors=['#5a8f6e','#b8903a','#c25f5f','#4a7fa5'],
                line=dict(color='#1c1f23',width=2)),textinfo='none'))
            fig2.update_layout(plot_bgcolor='rgba(0,0,0,0)',paper_bgcolor='rgba(0,0,0,0)',
                font=dict(color='#8a9099',family='DM Sans'),height=250,margin=dict(l=0,r=0,t=10,b=0),
                showlegend=True,legend=dict(orientation='v',font=dict(color='#8a9099',size=11),bgcolor='rgba(0,0,0,0)'),
    ...
    with qa1:
        if st.button("\u2295  New Reconciliation",use_container_width=True): st.session_state.step=1
    with qa2:
        rdir=Path("reports"); files=sorted(rdir.glob("*.xlsx"),key=lambda f:f.stat().st_mtime,reverse=True) if rdir.exists() else []
        if files:
            with open(files[0],'rb') as fh:
                st.download_button("\u2b07  Last Report",fh.read(),file_name=files[0].name,
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",use_container_width=True)
        else: st.button("\u2b07  Last Report",disabled=True,use_container_width=True)
    with qa3:
        if st.button("\u2715  Clear Session",use_container_width=True):
            st.session_state.reconciled_data=None; st.session_state.uploaded_df=None; st.session_state.step=1; st.rerun()
    st.markdown("</div>",unsafe_allow_html=True)
    ...
    if current==1:
        ...
        if uploaded:
            with tempfile.NamedTemporaryFile(delete=False,suffix=Path(uploaded.name).suffix) as tmp:
                tmp.write(uploaded.getvalue()); st.session_state.temp_file_path=tmp.name
            try:
                df=pd.read_csv(uploaded) if uploaded.name.endswith('.csv') else pd.read_excel(uploaded)
                st.session_state.uploaded_df=df; st.session_state.uploaded_file_name=uploaded.name
        ...
        df=st.session_state.uploaded_df; cols=df.columns.tolist()
        c1,c2=st.columns(2)
        with c1: cn_col=st.selectbox("Case Number Column",['-- select --']+cols)
        with c2: cit_col=st.selectbox("Citation Column",['-- select --']+cols)
        ...
        with bc3:
            ...
            if st.button("Continue \u2192",use_container_width=True):
                if cn_col=='-- select --' or cit_col=='-- select --': st.error("Select both columns.")
                else:
                    st.session_state.case_num_col=cn_col; st.session_state.citation_col=cit_col
        ...
        if not st.session_state.authenticated or not st.session_state.scrapper:
            st.error("Not connected to KRA iLaw. Please sign in first."); st.markdown("</div>",unsafe_allow_html=True); return
        workers=st.session_state.get('workers',8)
        pb=st.progress(0); msg=st.empty(); sub=st.empty()
        try:
            ...
            if not file_data: st.error("No data extracted."); st.markdown("</div>",unsafe_allow_html=True); return
            ...
            st.session_state.reconciled_data=reconciled; pb.progress(100)
            msg.markdown("<span style='color:#7ec89b'>\u2713 Reconciliation complete</span>",unsafe_allow_html=True); sub.empty()
            tot=len(reconciled)
            ...
            m1.metric("Total",tot); m2.metric("Verified",ver); m3.metric("Review",rev); m4.metric("Issues",iss)
            st.markdown('<div class="btn-primary">',unsafe_allow_html=True)
            if st.button("View Results \u2192",use_container_width=True): st.session_state.step=4; st.rerun()
            st.markdown('</div>',unsafe_allow_html=True)
        except Exception as e:
            st.error(f"Error: {e}"); errhandler(e,log="reconciliation",path="app"); traceback.print_exc()
        st.markdown("</div>",unsafe_allow_html=True)
    elif current==4:
        data=st.session_state.reconciled_data or []
        if not data:
            st.warning("No reconciliation data.")
            if st.button("Start New"): st.session_state.step=1; st.rerun()
            return
        total=len(data); verified=sum(1 for d in data if d.get('status')=='VERIFIED MATCH')
        review=sum(1 for d in data if d.get('status')=='REVIEW REQUIRED')
        mismatch=sum(1 for d in data if d.get('status')=='MISMATCH')
        nf=sum(1 for d in data if d.get('status')=='NOT FOUND')
        st.markdown("<div class='card'>",unsafe_allow_html=True)
        ...
        m1,m2,m3,m4=st.columns(4)
        m1.metric("Total",total); m2.metric("Verified",verified,f"{verified/total*100:.1f}%")
        m3.metric("Review",review,f"{review/total*100:.1f}%"); m4.metric("Issues",mismatch+nf,f"{(mismatch+nf)/total*100:.1f}%")
        rows=[]
        for d in data:
            cit=d.get('case_name',''); km=d.get('best_match_kra_citation','')
            rows.append({'Row':d.get('excel_row',''),'Case No.':d.get('original_case',''),
                'Citation':cit[:55]+'\u2026' if len(cit)>55 else cit,
                'Status':d.get('status',''),'Confidence':d.get('confidence_score',''),
                'KRA Match':km[:55]+'\u2026' if len(km)>55 else km,'KRA Ref':d.get('best_match_kra_ref','')})
        df_res=pd.DataFrame(rows)
        def color_status(s):
            p={'VERIFIED MATCH':'background-color:#1a3326;color:#7ec89b',
            ...
            if st.button("\u2b07 Excel Report",use_container_width=True):
                ...
                if fp and os.path.exists(fp):
                    with st.spinner("Building\u2026"):
                        if st.session_state.scrapper.report(data=data,file_path=fp): st.success("Saved to /reports")
                        else: st.error("Failed.")
                else: st.error("Source file missing.")
            st.markdown('</div>',unsafe_allow_html=True)
        with ec2:
            csv_b=df_res.to_csv(index=False).encode()
            st.download_button("\u2b07 Export CSV",csv_b,file_name=f"reconciliation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",mime="text/csv",use_container_width=True)
        with ec3:
            if st.button("\u2295 New Reconciliation",use_container_width=True):
                st.session_state.step=1; st.session_state.reconciled_data=None; st.session_state.uploaded_df=None; st.rerun()
        st.markdown("</div>",unsafe_allow_html=True)

def reports_page():
    st.markdown("<h1>Reports</h1>",unsafe_allow_html=True)
    rdir=Path("reports"); rdir.mkdir(exist_ok=True)
    files=sorted(list(rdir.glob("*.xlsx"))+list(rdir.glob("*.csv")),key=lambda f:f.stat().st_mtime,reverse=True)
    if not files:
        st.markdown("<div class='card'><p style='color:#8a9099;text-align:center;padding:3rem 0;'>No reports yet. Complete a reconciliation first.</p></div>",unsafe_allow_html=True); return
    ...
    with rc1: sel=st.selectbox("Report",[f.name for f in files],label_visibility="hidden")
    with rc2:
        rp=rdir/sel
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" if sel.endswith('.xlsx') else"text/csv"
        with open(rp,'rb') as fh: st.download_button("\u2b07 Download",fh.read(),file_name=sel,mime=mime,use_container_width=True)
    ...
    with ac1:
        ...
        if not st.session_state.chat_history:
            ...
            for m in st.session_state.chat_history:
                if m['role']=='ai': chat_html+=f"<div style='background:rgba(200,168,75,.06);border:1px solid rgba(200,168,75,.15);border-radius:0 8px 8px 8px;padding:.85rem 1rem;margin:.4rem 0;max-width:85%;color:#dde1e7;font-size:.87rem;'>{m['content']}</div>"
                else: chat_html+=f"<div style='background:rgba(255,255,255,.04);border:1px solid #2e333a;border-radius:8px 0 8px 8px;padding:.85rem 1rem;margin:.4rem 0 .4rem auto;max-width:85%;color:#dde1e7;font-size:.87rem;text-align:right;'>{m['content']}</div>"
        ...
        with st.form("cf",clear_on_submit=True):
            ...
            if st.form_submit_button("Send \u2192",use_container_width=True) and ui:
                ...
                st.session_state.chat_history.append({'role':'ai','content':resp}); st.rerun()
        st.markdown("</div>",unsafe_allow_html=True)
    with ac2:
        st.markdown("<div class='card'>",unsafe_allow_html=True)
        st.markdown("<h3>Suggested Questions</h3>",unsafe_allow_html=True)
        for q in ["What does VERIFIED MATCH mean?","Explain REVIEW REQUIRED","How is confidence scored?","Summarise my results","What courts are on KRA iLaw?","Help with column mapping"]:
...
def settings_page():
    ...
    with sc1:
            ...
            if st.form_submit_button("Update & Test",use_container_width=True):
                if username and password:
                    ...
                    with st.spinner("Testing\u2026"):
                        if ts.authenticator(): st.session_state.scrapper=ts; st.session_state.authenticated=True; st.success("Connection successful.")
            ...
            if not d_user or not d_pass:
                ...
                with st.spinner("Probing KRA iLaw login page…"):
                    try:
                        ...
                        r1=sess.get(AUTH_URL,timeout=15)
                        st.markdown(f"**Step 1 — GET login page**")
                        st.code(f"Status : {r1.status_code}\nURL    : {r1.url}",language="text")
                        soup=_BS(r1.text,"html.parser")
                        # Show all form inputs
                        inputs=[]
                        for inp in soup.find_all("input"):
                            inputs.append({"name":inp.get("name",""),"type":inp.get("type","text"),"id":inp.get("id",""),"value":inp.get("value","")[:40] if inp.get("value") else ""})
                        st.markdown("**Form inputs detected:**")
                        st.table(inputs)
                        # Step 2 — build payload from actual fields
                        hidden={}
                        ufield=pfield=None
                        SKIP={"redirect_to","csrf_token","_token","authenticity_token","token","submit"}
                        for inp in soup.find_all("input"):
                            ...
                            n=(inp.get("name") or "")
                            if t=="hidden" and n: hidden[n]=inp.get("value","")
                            if t=="password" and not pfield: pfield=n
                            if t in("text","email") and n.lower() not in SKIP and not ufield: ufield=n
                        if not ufield: ufield="username"
                        if not pfield: pfield="password"
                        ...
                        # Show meaningful snippet — strip scripts/styles
                        soup2=_BS(r2.text,"html.parser")
                        for tag in soup2(["script","style","head"]): tag.decompose()
        ...
        if st.button("Save",use_container_width=True): st.session_state.workers=w; st.success(f"Set to {w} workers.")
        st.markdown("</div>",unsafe_allow_html=True)
    with sc2:
        st.markdown("<div class='card'>",unsafe_allow_html=True)
        ...
        st.markdown(f"""<table style='width:100%;font-size:.82rem;color:#8a9099;border-collapse:collapse;'>
...
def main():
    ...
    _init()
    if not st.session_state.authenticated: login_page(); return
    selected=sidebar_nav()
    pages={"Dashboard":dashboard,"Reconciliation":reconciliation_page,"Reports":reports_page,"AI Assistant":ai_assistant_page,"Settings":settings_page}
